class LayerGraphicOptimized extends AGraphic implements IGISLayer {

    private static final Logger LOGGER = LogManager.getLogger(LayerGraphicOptimized.class);
    private static final IGISGraphic[] EMPTY_GRAPHICS = new IGISGraphic[0];

    private final Set<IGISLayerListener> listeners = new HashSet<>();
    protected final List<IGISGraphic> graphics = new ArrayList<>();
    ...
    public LayerGraphicOptimized(String name, IGISLayer parent) {
        super(name);
        if (parent != null) {
    ...
    void invalidateLayer() {
        if (getParent() != null) {
            if (getParent() instanceof LayerGraphicOptimized) {
                ((LayerGraphicOptimized) getParent()).invalidateLayer();
            } else if (getParent() instanceof LayerGraphic) {
    ...
    public IGISGraphic[] getChildren() {
        ...
        return graphics.toArray(new IGISGraphic[graphics.size()]);
    }
    ...
    private synchronized boolean add(int index, IGISGraphic child) {// TODO Birkan - Halil
        ...
        if (indexOfGraphicIdentity(child) >= 0) {
            return false;
        }
    ...
    private synchronized boolean remove(IGISGraphic child) {
        ...
        int index = indexOfGraphicIdentity(child);
    ...
    private int indexOfGraphicIdentity(IGISGraphic child) {
        if (child == null) {
            return -1;
        }

        for (int i = 0, size = graphics.size(); i < size; i++) {
            if (graphics.get(i) == child) {
                return i;
            }
        }
    ...
    public int indexOf(IGISGraphic child) {
        return indexOfGraphicIdentity(child);
    }
    ...
    private void getPrimitives(ArrayList<IGISGraphic> avisibleGraphicList, IGISGraphic graphic) {
        if (graphic instanceof LayerGraphicOptimized) {
            List<IGISGraphic> childGraphics = ((LayerGraphicOptimized) graphic).graphics;
            for (int i = 0, size = childGraphics.size(); i < size; i++) {
                getPrimitives(avisibleGraphicList, childGraphics.get(i));
            }
        } else if (graphic instanceof IGISLayer) {
    ...
    public void gridChilds(int matrixSize) {
        ...
        List<IGISGraphic> children = new ArrayList<>(graphics);
        ArrayList<IGISGraphic> childrenVector = new ArrayList<>(children);
        ...
        for (IGISGraphic child : children) {
            ...
            if (group == null) {
                IGISLayer groupagg = new LayerGraphicOptimized("Group " + key, null);
                groups.put(key, groupagg);
                group = new ArrayList<>();
                groupItems.put(key, group);
    ...
    public void createOverview() {
        ...
        List<IGISGraphic> children = graphics;
        ArrayList<GISPosition[]> overviewPointsPolygon = new ArrayList<>();
        ArrayList<GISPosition[]> overviewPointsPolyline = new ArrayList<>();
        IGISPolygonSymbolizer overviewPolygonSymbolizer = null;
        IGISLineSymbolizer overviewLineSymbolizer = null;
    ...
    public void setVisible(boolean visible) {
        for (int i = 0, size = graphics.size(); i < size; i++) {
            graphics.get(i).setVisible(visible);
        }
    ...
    private final Map<Font, ArrayList<IGISGraphicLabel>> fontLabelMap = new HashMap<>();
    private final Map<ScaledFontKey, Font> scaledFontCache = new HashMap<>();
...
//                                }
    ...
    private void updateFontMap(IGISMap2D map, IGISGraphicLabel[] labels, IGISGraphicIcon icon) {
        boolean autoScaleEnabled = ((GLRendererGraphicIcon) (map.getRendererManager().getRenderer(IGISGraphicIcon.GRAPHIC_TYPE))).isAutoScaleEnabled();
        for (IGISGraphicLabel label : labels) {
            label.setPosition(icon.getPosition());
            Font font = label.getTextSymbolizer().getFont();
            if (autoScaleEnabled) {
                float scaledSize = (float) (font.getSize() / (map.getScale() * 14));
                font = getScaledFont(font, scaledSize);
            }
            ArrayList<IGISGraphicLabel> iconList = fontLabelMap.get(font);
            if (iconList == null) {
    ...
    private Font getScaledFont(Font baseFont, float scaledSize) {
        ScaledFontKey key = new ScaledFontKey(baseFont, scaledSize);
        Font scaledFont = scaledFontCache.get(key);
        if (scaledFont == null) {
            scaledFont = baseFont.deriveFont(scaledSize);
            scaledFontCache.put(key, scaledFont);
        }
        return scaledFont;
    }
    ...
    public List<IGISGraphic> getGraphics() {
        ...
        for (IGISGraphic graphic : graphics) {
            if (graphic instanceof LayerGraphicOptimized) {
                ((LayerGraphicOptimized) graphic).getGraphics(result);
            } else if (graphic instanceof IGISLayer) {
    ...
    private static final class ScaledFontKey {

        private final Font baseFont;
        private final int scaledSizeBits;

        private ScaledFontKey(Font baseFont, float scaledSize) {
            this.baseFont = baseFont;
            this.scaledSizeBits = Float.floatToIntBits(scaledSize);
        }
        ...
        public boolean equals(Object obj) {
            ...
            if (!(obj instanceof ScaledFontKey)) {
                return false;
            }
            ScaledFontKey other = (ScaledFontKey) obj;
            return scaledSizeBits == other.scaledSizeBits && baseFont.equals(other.baseFont);
        }

        @Override
        public int hashCode() {
            return 31 * baseFont.hashCode() + scaledSizeBits;
        }
    }
}
